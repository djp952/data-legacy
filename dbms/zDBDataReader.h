...
#ifndef __ZDBDATAREADER_H_
#define __ZDBDATAREADER_H_
#pragma once
...
public ref class zDBDataReader sealed : public DbDataReader, public ITrackableObject
{
public:
	...
	// GetBinaryReader
	//
	// Retrieves the specified value as a special zDBBinaryReader object
...
internal:
	...
	zDBDataReader(zDBCommand^ command, zDBQuery^ query, zDBCommandBehavior behavior);
	zDBDataReader(zDBCommand^ command, String^ query, zDBCommandBehavior behavior);
...
private:
	...
	//
	// Simple helper function used to check a zDBStatement status
	static void CheckStatementStatus(zDBStatement^ statement);

	// IsCommandBehavior
	//
	// Determines if the specified command behavior flag is set
	bool IsCommandBehavior(zDBCommandBehavior behavior);
	...
	zDBConnection^				m_conn;				// Database connection
	__int64						m_cookie;			// Registration cookie
	zDBQuery^					m_query;			// Contained compiled query
	zDBCommandBehavior			m_behavior;			// Reader behavior flags
	bool						m_disposeQuery;		// Dispose of query on close?
	int							m_stmtIndex;		// Current statement index
	zDBStatement^				m_stmt;				// Current statement object
	int							m_changes;			// Overall records affected
	zDBParameterCollection^		m_params;			// Contained parameters collection
};
