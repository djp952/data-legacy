...
//---------------------------------------------------------------------------
// zdbutil_scalar_callback
//
// Callback used for zDBUtil::ExecuteScalar
//
// Arguments:
//
...
int zdbutil_scalar_callback(void *context, int argc, char* argv[], char* azColName[])
{
	GCHandleRef<zDBUtil::ScalarResultObject^> contextObject(context);
...
//---------------------------------------------------------------------------
// zDBUtil::CheckConnectionClosed (static)
//
// Determines if the zDBConnection instance is "closed", and throws an 
// exception if it isn't
//
// Arguments:
//
//	conn		- zDBConnection instance to be checked

void zDBUtil::CheckConnectionClosed(zDBConnection^ conn)
{
...
//---------------------------------------------------------------------------
// zDBUtil::CheckConnectionOpen (static)
//
// Determines if the zDBConnection instance is "open", and throws an 
// exception if it isn't
//
// Arguments:
//
...
void zDBUtil::CheckConnectionOpen(zDBConnection^ conn)
{
	CheckConnectionValid(conn);
...
//---------------------------------------------------------------------------
// zDBUtil::CheckConnectionReady (static)
//
// Determines if a zDBConnection is in a "ready" state, meaning it can
...
void zDBUtil::CheckConnectionReady(zDBConnection^ conn)
{
	CheckConnectionValid(conn);
...
//---------------------------------------------------------------------------
// zDBUtil::CheckConnectionValid (static)
//
// Determines if a zDBConnection is valid or not.  A null pointer, a disposed
...
void zDBUtil::CheckConnectionValid(zDBConnection^ conn)
{
...
//---------------------------------------------------------------------------
// zDBUtil::CheckDataRecordOrdinal (static)
//
...
void zDBUtil::CheckDataRecordOrdinal(IDataRecord^ record, int ordinal)
{
...
////---------------------------------------------------------------------------
//// zDBUtil::ColumnDefinitionToType (static)
////
...
//
//Type^ zDBUtil::ColumnDefinitionToType(String^ definition)
//{
...
//---------------------------------------------------------------------------
// zDBUtil::EncodingToPragma (static)
//
// Converts a zDBTextEncodingMode value into a string that can be used in
...
String^ zDBUtil::EncodingToPragma(zDBTextEncodingMode encoding)
{
	switch(encoding) {

		case zDBTextEncodingMode::UTF8: return "UTF-8";
		case zDBTextEncodingMode::UTF16: return "UTF-16";
		case zDBTextEncodingMode::UTF16LittleEndian: return "UTF-16le";
		case zDBTextEncodingMode::UTF16BigEndian: return "UTF-16be";
	}
...
//---------------------------------------------------------------------------
// zDBUtil::ExecuteNonQuery (static)
//
...
void zDBUtil::ExecuteNonQuery(sqlite3* hDatabase, String^ query)
{
...
// Arguments:
//
//	conn		- zDBConnection instance to execute against
//	query		- SQL query to be executed against the connection

String^ zDBUtil::ExecuteScalar(sqlite3* hDatabase, String^ query)
{
	...
	nResult = sqlite3_exec(hDatabase, AutoAnsiString(query), zdbutil_scalar_callback, 
...
//---------------------------------------------------------------------------
// zDBUtil::FastPtrToStringAnsi (static)
//
...
String^ zDBUtil::FastPtrToStringAnsi(const char *psz)
{
...
String^ zDBUtil::FastPtrToStringAnsi(const char *psz, size_t cch)
{
...
//---------------------------------------------------------------------------
// zDBUtil::PragmaToEncoding (static)
//
// Converts a PRAGMA ENCODING result string into a zDBTextEncodingMode value
//
// Arguments:
//
//	pragma		- The result from a PRAGMA ENCODING query

zDBTextEncodingMode zDBUtil::PragmaToEncoding(String^ pragma)
{
	...
	if(String::Compare(pragma, "UTF-8", true) == 0) return zDBTextEncodingMode::UTF8;
	else if(String::Compare(pragma, "UTF-16le", true) == 0) return zDBTextEncodingMode::UTF16LittleEndian;
	else if(String::Compare(pragma, "UTF-16be", true) == 0) return zDBTextEncodingMode::UTF16BigEndian;
	else if(String::Compare(pragma, "UTF-16", true) == 0) return zDBTextEncodingMode::UTF16;

	else throw gcnew ArgumentOutOfRangeException();		// Invalid value
}

//---------------------------------------------------------------------------
// zDBUtil::ValidateDataSource (static)
//
...
bool zDBUtil::ValidateDataSource(String^ dataSource)
{
	Debug::Assert(dataSource != nullptr);

	if(String::Compare(dataSource, zDBDataSource::Memory, true) == 0) return true;
	return ValidateFileName(dataSource);
}

//---------------------------------------------------------------------------
// zDBUtil::ValidateFileName (static)
//
...
bool zDBUtil::ValidateFileName(String^ path)
{
...
//---------------------------------------------------------------------------
// zDBUtil::ValidateFolderName (static)
//
// Validates that the specified path does not contain any invalid characters
//
// Arguments:
...
bool zDBUtil::ValidateFolderName(String^ path)
{
	Debug::Assert(path != nullptr);
